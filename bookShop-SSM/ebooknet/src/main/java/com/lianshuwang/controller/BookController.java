...
public class BookController {
    ...
    public void getBookCover(String coverPath, HttpServletResponse response) {
        ...
        try {
            ...
            // 后来找的解决方法
//            FileInputStream is = new FileInputStream(coverPath);
...
//            toClient.close();
    ...
    public void getBookDownload(long bookID, String filePath, HttpServletResponse response) {
        ...
        Book book;
        book = bookService.getBookDetail(bookID);
        String newfilePath = book.getBook_file();
        response.setContentType("text/html;charset=utf-8");
        String fileName = newfilePath.substring(newfilePath.lastIndexOf("/") + 1);
        BufferedInputStream bis = null;
        BufferedOutputStream bos = null;
        try {
            long fileLength = new File(newfilePath).length();
            response.setContentType("application/x-msdownload");
            response.setHeader("Content-disposition", "attachment; filename="
                    + new String(fileName.getBytes("utf-8"), "ISO8859-1"));
            response.setHeader("Content-Length", String.valueOf(fileLength));
            bis =new BufferedInputStream(new FileInputStream(newfilePath));
